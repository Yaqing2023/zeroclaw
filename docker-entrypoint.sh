#!/bin/sh
# ZeroClaw Docker entrypoint v9 - proper permission handling with gosu
# Version 9: Start as root, fix volume permissions, drop to unprivileged user
...
echo "[entrypoint v9] Starting as $(id)..."

# ── Fix volume permissions (must run as root) ─────────────────
if [ "$(id -u)" = "0" ]; then
    echo "[entrypoint v9] Fixing /zeroclaw-data ownership for user $RUN_USER..."
    chown -R $RUN_USER:$RUN_GROUP /zeroclaw-data
    echo "[entrypoint v9] Permissions fixed, dropping to user $RUN_USER..."
...
echo "[entrypoint v9] Running as $(id)"
...
echo "[entrypoint v9] Writing MoltsPay env vars to workspace/.env..."
...
if [ ! -f "$MOLTSPAY_DIR/wallet.json" ]; then
    echo "[entrypoint v9] Initializing MoltsPay wallet..."
    ...
    npx moltspay init --chain base --max-per-tx 100 --max-per-day 1000 2>&1 || {
        echo "[entrypoint v9] Warning: moltspay init failed, will retry later"
    }
    ...
    if [ -f "$MOLTSPAY_DIR/wallet.json" ] && [ -n "$MOLTSPAY_API_URL" ] && [ -n "$AGENT_TOKEN" ]; then
        ...
        echo "[entrypoint v9] Reporting wallet address: $WALLET_ADDRESS"
        curl -s -X POST "$MOLTSPAY_API_URL/api/internal/agents/wallet" \
            -H "Authorization: Bearer $AGENT_TOKEN" \
            -H "Content-Type: application/json" \
            -d "{\"address\": \"$WALLET_ADDRESS\"}" || {
            echo "[entrypoint v9] Warning: Failed to report wallet address"
        }
    fi
else
    echo "[entrypoint v9] MoltsPay wallet already exists"
fi

# ── Fetch and apply spending limit from backend ──────────────────
if [ -n "$MOLTSPAY_API_URL" ] && [ -n "$AGENT_TOKEN" ]; then
    echo "[entrypoint v9] Fetching spending config from backend..."
    ...
    if [ -n "$CONFIG_RESPONSE" ]; then
        ...
        if [ -n "$SPENDING_LIMIT" ] && [ "$SPENDING_LIMIT" != "null" ]; then
            echo "[entrypoint v9] Applying spending limit: \$$SPENDING_LIMIT/day"
            npx moltspay config --max-per-day "$SPENDING_LIMIT" 2>&1 || {
                echo "[entrypoint v9] Warning: Failed to apply spending limit"
            }
        fi
    fi
fi
...
echo "[entrypoint v9] Generating fresh config.toml..."

# Build Discord config inline if set
DISCORD_INLINE=""
if [ -n "$ZEROCLAW_DISCORD_BOT_TOKEN" ]; then
    echo "[entrypoint v9] Building Discord config..."
...
echo "[entrypoint v9] Config generated:"
echo "----------------------------------------"
cat "$CONFIG_FILE"
echo "----------------------------------------"
echo "[entrypoint v9] Starting ZeroClaw..."

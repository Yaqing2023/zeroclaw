#!/bin/sh
# ZeroClaw Docker entrypoint v6 - fixed moltspay init
# Version 6: Remove invalid --yes flag, use explicit --max-per-tx/--max-per-day
...
echo "[entrypoint v6] Starting..."

# ── Write MoltsPay env vars to workspace/.env for agent shell access ──────
echo "[entrypoint v6] Writing MoltsPay env vars to workspace/.env..."
...
ENVEOF
echo "[entrypoint v6] Env vars written to $WORKSPACE_DIR/.env"

# ── Initialize MoltsPay wallet if not exists ──────────────────
if [ ! -f "$MOLTSPAY_DIR/wallet.json" ]; then
    echo "[entrypoint v6] Initializing MoltsPay wallet..."
    ...
    npx moltspay init --chain base --max-per-tx 100 --max-per-day 1000 2>&1 || {
        echo "[entrypoint v6] Warning: moltspay init failed, will retry later"
    }
    ...
    if [ -f "$MOLTSPAY_DIR/wallet.json" ] && [ -n "$MOLTSPAY_API_URL" ] && [ -n "$AGENT_TOKEN" ]; then
        ...
        echo "[entrypoint v6] Reporting wallet address: $WALLET_ADDRESS"
        curl -s -X POST "$MOLTSPAY_API_URL/api/internal/agents/wallet" \
            -H "Authorization: Bearer $AGENT_TOKEN" \
            -H "Content-Type: application/json" \
            -d "{\"address\": \"$WALLET_ADDRESS\"}" || {
            echo "[entrypoint v6] Warning: Failed to report wallet address"
        }
    fi
else
